...
| tidal | `/home/fabio/tidal` + `~/tidal-foundry` (git) | **TidalCycles foundry** — constitution (rubric ≥6/8, seeds immutable, provenance), 23 seeds, 7 concept hubs, `knowledge/repo-map.md` = 30-repo mining index (⬜ open: vroomvroom deep study, looper eval, JP 100-one-liners batch, Tier 2/3). Canonical home of `~/.config/nvim/knowledge` (symlink). Headless rig per `[[FOUNDRY]]` there. Umbrella repo `~/tidal-foundry` (2026-09-07): campaigns (tidal-swarm absorbed, lane histories as bundles), briefs, clippings — canonical map in its README. |
